...
public class Boss : MonoBehaviour
{
    ...
    private IEnumerator TentacleLineAttack()
    {
        ...
        // **�nce k�rm�z� uyar� efekti verelim**
        foreach (Transform tentacle in tentacles)
        {
            SpriteRenderer sr = tentacle.GetComponent<SpriteRenderer>();
            if (sr != null)
            {
                sr.color = Color.red; // **K�rm�z� uyar�**
            }
        }

        yield return new WaitForSeconds(attackDelay); // **Uyar� s�resi**

        // **Uyar�y� kald�r ve sald�r�y� yap**
        foreach (Transform tentacle in tentacles)
        {
            SpriteRenderer sr = tentacle.GetComponent<SpriteRenderer>();
            if (sr != null)
            {
                sr.color = Color.black; // **Eski rengine d�n**
            }
